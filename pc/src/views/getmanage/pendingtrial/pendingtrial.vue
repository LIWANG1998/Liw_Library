    ...
    <Maincontent>
      ...
      <div class="head">
        ...
        <div class="right">
          ...
          <a-icon type="more" />
          <a-button class="btn" @click="addDocumentExcelout" :disabled="comIds">导出</a-button>
          <a-button v-if="powers('post_print')" class="btn" @click="batchPrinting(1)">批量打印</a-button>
        </div>
      </div>
      <SeniorSearch class="SeniorSearch" :obj="obj" @oninSearch="oninSearch" ref="searchDetail" />
      <div class="flex-row-db">
        <div class="table-content-at" :style="{height:`${height + 54 + 12 +12}px`}">
          <a-table
            :columns="columns"
            :pagination="false"
            :data-source="data"
...
export default {
  ...
  data() {
    return {
      ...
      // 模板弹窗数据
      TemplatePopup: { visibles: false, type: 2, title: "收文模板", type2: 2 },
      //页数配置
      paging: {
        current: 1,
        total: null,
  ...
  mounted() {
    this.height = Calculatedaltitude(60, 24, 100, 73);
  },
  methods: {
    // tabs切换事件
    onCallback(id) {
    ...
    addlist() {
      ...
      document_wait_approval_list({
        ...
        advanced: {
          ...
          copy_users: obj.one["收文单位"]?.intext, // 收文、签报单位
  ...
  computed: {
    // 获取权限
    jurisdiction() {
      return this.$store.state.jurisdiction[2];
    },
    comIds() {
      return this.ids.ids == "";
    }
