...
<template>
  <div class="maincontent-tab">
    <div class="maincontent-tab-col">
      <div class="maincontent-tab-col-title maincontent-tab-col-title-h">模板名称</div>
    ...
    <div class="maincontent-tab-col">
      <div class="maincontent-tab-col-title maincontent-tab-col-title-h">收文类别名称</div>
      <div class="maincontent-tab-col-content">
        <a-form-model-item label class="item-w">
          <div class="flexIcon">
            <a-radio-group v-model="genre" class="choiseType">
              <a-radio
                class="choiseRadio"
                :value="item.id"
              ...
              <div class="right">
                <div class="top">
                  ...
                  <div class="person-list">
                    <div class="person-item" v-for="(item,index) in approved" :key="index">
                      ...
                      <div class="msg">
                        <div class="user-img">
                          ...
                          <!-- <i class="x hand" @click="appitemClose(item.value)"></i> -->
                        </div>
                    ...
                    <!-- <div class="add-user" @click="appvisi=true">+</div> -->
                  </div>
                </div>
                <div class="bottom">
                    ...
                    <div class="person-item" v-for="(item,index) in CCroved" :key="index">
                      <div class="msg">
                        <div class="user-img">
                          ...
                          <!-- <i class="x hand" @click="CCitemClose(item.value)"></i> -->
                        </div>
                        <div class="name">{{item.title}}</div>
                      </div>
                    </div>
                    <!-- <div class="add-user" @click="CCperson=true">+</div> -->
                  </div>
                </div>
              </div>
            </div>
    ...
    <div class="maincontent-tab-col borderTom">
      ...
      <div class="maincontent-tab-col-content">
        <a-form-model-item label class="item-w">
          <a-radio-group style="margin:18px 0 15px;" v-model="is_red" @change="onCrimson">
...
export default {
  ...
  watch: {
    Listdata(newValue, oldValue) {
      if (newValue) {
        this.tepName = newValue.temp_name;
        this.genre = Number(newValue.type_id);
        this.liuId = newValue.technological_process_id;
        if (newValue.word_no_id != null) {
          this.word_no_id = Number(newValue.word_no_id);
          this.word_nos = Number(newValue.word_no_id);
          this.role_data.more_role = true;
        } else {
          this.word_no_id = undefined;
        }
        this.is_red = newValue.is_red;
        if (newValue.red_template_id != null) {
          let template = newValue.red_template_id.split(",");
          this.red_template = template.map(Number);
        }
        if (this.is_red == 1) {
          this.whethers = true;
        }
  ...
  created() {
    ...
    this.getallList();
    // 点击保存发送给主页面数据
    this.$bus.$on("compileworksetgetdocument", (val) => {
      this.$bus.$emit("compileworksetgetbasicinfo", {
        type_id: this.genre,
        word_no_id: this.word_no_id,
        process_id: this.liuId,
        is_red: this.is_red,
        temp_name: this.tepName,
        red_template: this.red_template,
      });
    });
  },
  data() {
  ...
  methods: {
    ...
    handleOk() {
      if (this.status == 1) {
        this.$router.push(`/getclassifyset`);
      } else if (this.status == 2) {
        this.$router.push(`/getruleset/addressee`);
      } else if (this.status == 3) {
        this.$router.push(`/getflowpathset/getflowpathadd`);
      } else if (this.status == 4) {
        this.$router.push(`/getcrimsonset/getcrimsonadd?type=1`);
      }
    },
    lookMare() {
    ...
    //模板名称
    onName(e) {
      this.$emit("compilegetformname", e.target.value);
    },
    //发文的类别
    onGenre(e) {
      this.$emit("compilegetformonGenre", e.target.value);
    },
    //规则
    onRule(e) {
      this.$emit("compilegetformonRule", e.target.value);
    ...
    //流程处理
    onProcedure(e) {
      this.$emit("compilegetformonProcedure", e);
      this.liuId = e;
      this.getLiuDetail();
    },
    //是否套红
    onCrimson(e) {
      this.$emit("compilegetformonCrimson", e.target.value);
      if (e.target.value == 0) {
        this.whethers = false;
      } else {
        this.whethers = true;
    ...
    onCrimsonName(e) {
      this.$emit("compilegetformonCrimsonName", e.join());
    },
    // 获取类别 字号 流程 套红列表
    getallList() {
      GetAllList({ type: 2 }).then((res) => {
        ...
        if (this.word_no_id != null) {
          this.allWordnoList.forEach((el) => {
            if (el.id === this.word_no_id) {
              this.role_data.zanshi_word_no = el.word_no;
            }
          });
        }
