...
export default {
  ...
  methods: {
    ...
    //模板名称
    onName(e) {
      this.$emit("getformaddname", e.target.value);
    },
    //发文的类别
    onGenre(e) {
      this.$emit("getformaddonGenre", e.target.value);
    },
    //规则
    onRule(e) {
      this.$emit("getformaddonRule", e.target.value);
    ...
    //流程处理
    onProcedure(e) {
      this.$emit("getformaddonProcedure", e);
      this.liuId = e;
      this.getLiuDetail();
    },
    //是否套红
    onCrimson(e) {
      this.$emit("getformaddonCrimson", e.target.value);
      if (e.target.value == 0) {
        this.whethers = false;
      } else {
        this.whethers = true;
    ...
    onCrimsonName(e) {
      this.$emit("getformaddonCrimsonName", e.join());
    },
    // 获取类别 字号 流程 套红列表
    getallList() {
