...
export default {
  ...
  created() {
    ...
    this.$bus.$on("compileworksetgetdocument", (val) => {
      this.$bus.$emit("signoffformworksetcompileworksetgetdocument", {
        type_id: this.genre,
  ...
  methods: {
    ...
    //模板名称
    onName(e) {
      this.$emit("signoffformworksetcompilename", e.target.value);
    },
    //发文的类别
    onGenre(e) {
      this.$emit("signoffformworksetcompileonGenre", e.target.value);
    },
    //规则
    onRule(e) {
      this.$emit("signoffformworksetcompileonRule", e.target.value);
    ...
    //流程处理
    onProcedure(e) {
      this.$emit("signoffformworksetcompileonProcedure", e);
      this.liuId = e;
      this.getLiuDetail();
    },
    //是否套红
    onCrimson(e) {
      this.$emit("signoffformworksetcompileonCrimson", e.target.value);
      if (e.target.value == 0) {
        this.whethers = false;
      } else {
        this.whethers = true;
    ...
    onCrimsonName(e) {
      this.$emit("signoffformworksetcompileonCrimsonName", e.join());
    },
    // 获取类别 字号 流程 套红列表
    getallList() {
