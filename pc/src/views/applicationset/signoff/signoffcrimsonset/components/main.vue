  ...
  methods: {
    ...
    pagechangemsonset(val) {
      ...
      if (this.getpostList) {
        this.onList();
        // 让查询的数据也改变
        this.$bus.$emit("signoffsetpostList", {
    ...
    // 获取查询后的列表
    onList() {
      this.$bus.$on("signoffloadList", (val) => {
        if (val.res.data) {
          let arr = [];
          val.res.data.map((v) => {
            let obj = {};
    ...
    ChangeSort(p, f, s) {
      if (s.order) {
        ...
        this.$bus.$emit("signcrimsonsetsort", {
          sort: order,
          field: s.columnKey,
        });
        if (this.getpostList) {
