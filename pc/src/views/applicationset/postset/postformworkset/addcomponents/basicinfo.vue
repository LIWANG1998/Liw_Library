...
export default {
  ...
  methods: {
    ...
    //发文的类别
    onGenre(e) {
      this.$emit("onGenre", e.target.value);
    },
    //规则
    onRule(e) {
      this.$emit("onRule", e.target.value);
    ...
    //流程处理
    onProcedure(e) {
      this.$emit("onProcedure", e);
      this.liuId = e;
      this.getLiuDetail();
    },
    //是否套红
    onCrimson(e) {
      this.$emit("onCrimson", e.target.value);
      if (e.target.value == 0) {
        this.whethers = false;
      } else {
        this.whethers = true;
    ...
    onCrimsonName(e) {
      this.$emit("onCrimsonName", e.join());
    },
    // 获取类别 字号 流程 套红列表
    getallList() {
