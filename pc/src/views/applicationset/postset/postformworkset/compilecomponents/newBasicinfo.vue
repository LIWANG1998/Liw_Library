...
<template>
  <div class="maincontent-tab">
    ...
    <div class="maincontent-tab-col">
      <div class="maincontent-tab-col-title maincontent-tab-col-title-h">发文类别名称</div>
...
export default {
  ...
  methods: {
    ...
    handleOk() {
      if (this.status == 1) {
        this.$router.push(`/postclassifyset`);
    ...
    //模板名称
    onName(e) {
      this.$emit("compilename", e.target.value);
    },
    //发文的类别
    onGenre(e) {
      this.$emit("compileonGenre", e.target.value);
    },
    //规则
    onRule(e) {
      this.$emit("compileonRule", e.target.value);
    ...
    //流程处理
    onProcedure(e) {
      this.$emit("compileonProcedure", e);
      this.liuId = e;
      this.getLiuDetail();
    },
    //是否套红
    onCrimson(e) {
      this.$emit("compileonCrimson", e.target.value);
      if (e.target.value == 0) {
        this.whethers = false;
      } else {
        this.whethers = true;
    ...
    onCrimsonName(e) {
      this.$emit("compileonCrimsonName", e.join());
    },
    // 获取类别 字号 流程 套红列表
    getallList() {
      GetAllList({ type: 1 }).then((res) => {
