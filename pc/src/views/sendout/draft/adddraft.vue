<template>
  <div>
    ...
    <Maincontent>
      ...
      <Enclosure class="mb" />
      <Footer :all_data="data" class="mb" @SettingNotice="SettingNotice" />
      <Setting
        v-show="is_show"
        :all_data="data"
        class="mb"
      ...
      <a-affix :offset-bottom="0">
        <div class="btn-bg flex-col">
          ...
          <a-button
            ...
            type="primary"
            @click="referto"
            >{{ $route.query.can_edit ? "重新提交" : "提交" }}
          </a-button>
          <a-dropdown placement="topCenter">
            <a-button style="margin-left: 20px">
              <a-icon class="more" type="unordered-list" />
              更多
            </a-button>
            <a-menu slot="overlay">
...
export default {
  ...
  methods: {
    ...
    async go_document_add(status = "", NoJump = "") {
      ...
      // 公文分发记录id，
      let distribute_id = "";
      if (this.$route.query.distribute_id) {
    ...
    async get_distribute_file_detail() {
      ...
      data2.data.fields.forEach((el) => {
        ...
        if (el.e_name == "word_no") {
          el.value = { id: data2.data.word_no_id, word_no: data2.data.word_no };
        }
        head[el.e_name] = el;
      });
      this.radio_data = {
      ...
      data.fields.forEach((el) => {
        ...
        if (el.e_name == "word_no") {
          head[el.e_name].value = {
            id: data2.data.word_no_id,
            word_no: data2.data.word_no,
          };
        }
      });
      ...
      //设置字号
      this.$store.commit("SET_DOCUMENT_WORD_NO", data2.data.word_no);
      this.$store.commit("SET_DOCUMENT_WORD_NO_ID", data2.data.word_no_id);
