    ...
    <div class="addInpostrafts-enclosure-textbox">
      ...
      >
        <div class="left">
          ...
          <div class="msg">
            <span class="addInpostrafts-enclosure-textbox-inbox-tltle-text">{{
              item.oldname
            }}</span>
            <span class="addInpostrafts-enclosure-textbox-inbox-tltle-file-size"
              >{{ item.size }}K</span
            >
          </div>
        </div>
...
export default {
  ...
  components: {},
  created() {
    this.$bus.$off("document_files");
    this.$bus.$on("document_files", this.set_files);
  },
  computed: {
    get_files() {
  ...
  methods: {
    ...
    //向vuex发送数据
    set_files() {
      this.$store.commit("SET_DOCUMENT_FILES", this.vuex_percent);
    },
    // 判断上传文件是否符合要求
    beforeUpload(file) {
      let _type = [
    ...
    quxiao() {
      ...
      if (this.false_percent.length !== this.vuex_percent.length) {
        if (this.false_percent.length > 0) {
          this.quxiao_id = this.false_percent[
            this.false_percent.length - 1
          ].uid;
        }
      } else {
    ...
    //清空内容
    reste_data() {
      this.$store.commit("RESET_SET_DOCUMENT_FILES");
      this.vuex_percent = [];
    },
  },
  watch: {
