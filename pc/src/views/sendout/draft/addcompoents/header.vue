  ...
  methods: {
    ...
    handleOk_ren() {
      ...
      this.all_data[this.that_e_name] = this.all_data[this.that_e_name] =
        ...
        this.all_data[this.that_e_name].length > 0
          ...
          : [];
      console.log(this.select_data, this.all_data);
      if (this.that_e_name == "main_delivery") {
        // ---------------------这个地方有问题--------------------
        let revi =
          this.that_e_name == "main_delivery" ? "reviewer" : "receiving_user";
        this.select_data[revi].length > 0 ? [...this.select_data[revi]] : [];
        //设置接收人
      ...
      //   }
      console.log(this.all_data, this.select_data);
      this.set_vuex_data();
    },
    ...
    set_vuex_data() {
      ...
      for (let key in this.select_data) {
        ...
        if (key == "main_delivery" || key == "receiving_unit") {
          let ss_arr = JSON.parse(JSON.stringify(this.select_data[key]));
          let select = [];
          let inter = [];
          ss_arr &&
            ss_arr.forEach((el) => {
    ...
    main_delivery_deselect(val, nem) {
      ...
      this.select_data[nem].forEach((el, inx) => {
        if (el.name == val) {
          let receiver_name = this.select_data[nem][inx].receiver_name;
          let receiver_v = this.select_data[nem][inx].id;
          this.select_data[nem].splice(inx, 1);
          this.all_data[nem].splice(inx, 1);
          for (
            let index = this.select_data[revi].length - 1;
            index >= 0;
            index--
          ) {
            console.log(this.select_data[revi]);
            if (this.select_data[revi][index].id == receiver_v) {
              this.all_data[revi].forEach((ite, dex) => {
                if (ite === this.select_data[revi][index].name) {
                  this.all_data[revi].splice(dex, 1);
                }
              });
              this.select_data[revi].splice(index, 1);
            }
          }
        }
      });
    ...
    get_father_data() {
          ...
          for (let key in this.head_need_data) {
            ...
            if (
              ...
              if (this.head_need_data[key].value) {
                ...
                if (
                  this.head_need_data[key].value.inter &&
                  this.head_need_data[key].value.inter.length &&
                  key !== "word_no"
                ) {
                  this.head_need_data[key].value.inter.forEach((el) => {
                    a_value.push(el.name);
                    sel_val.push(el);
                  });
                }
      ...
      this.$store.commit("SET_DOCUMENT_HEADER", this.head_need_data);
      if (
        (this.$route.query.type == 1 || this.$route.query.type == 3) &&
        this.get_model
      ) {
