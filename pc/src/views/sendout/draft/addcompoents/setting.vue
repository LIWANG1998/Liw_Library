<template>
  <div class="setting"
      ...
      <div class="s-content">
        <div class="c-title">
          <div class="flex-align-center"
               v-if="process.that_process">
            当前流程：
            <span class="that-process">{{process.that_process.name}}</span>

            <span class="set-collection hand"
          ...
          <span class="set-process hand"
                @click="set_new_process(1)">设置新的流程</span>
        </div>
        <div class="c-content">
          <div class="left">
            <span class="use-liucheng">常用流程：</span>
            <div class="liucheng-box">
              <a-radio-group @change="liucheng_change"
                ...
                <div class="ofen-use-list">
                  <a-radio :value="item.id"
                           ...
                           :key="index"
                           :style="get_process.liucheng_id==item.id?'color:#0082ef;':''">
                    <span class="text"
                  ...
                  <div class="more"
                    ...
                    <span>更多流程</span>
                    <i class="sanjiao"
                       :class="more_show_liucheng?'shang':'xia'"></i>
                  </div>
                </div>

                <div class="more-content"
                     v-show="more_show_liucheng">
                  <div class="liucheng-search">
                    <a-input-search placeholder="搜索流程名称"
                                    style="width: 279px"
                  ...
                  <div class="radio-list"
                    ...
                    <div class="other-radio">
                      <a-radio @click="process_change(item)"
                               :value="item.id"
                               v-for="(item,index) in process.all_process"
                               :key="index"
                               :style="get_process.liucheng_id==item.id?'color:#0082ef;':''">
                        {{item.name}}
                        <span v-if="item.line==1"
                              class="ofen-use-icon">常用</span>
                      </a-radio>
                    </div>
                  </div>
                </div>
            ...
            <div class="top">
              ...
              <div class="person-list">
                <div class="person-item"
                  ...
                  <div class="msg"
                       @click="add_approval(3,index)"
                       v-if="item.info.length>=1">
                    <div class="user-img">
                      <div v-if="item.type==3"
                           class="head-img">
                        <img :src="item.info[0].avatar||require('../../../../assets/adddraft/default-head-img.png')"
                             alt />
                      </div>
                      <div v-else
                           class="head-img">
                        <img :src="require('../../../../assets/adddraft/default-head-img.png')"
                             alt />
                      </div>
                      <i class="x hand"
                         v-if="process.process_jurisdiction&&process.process_jurisdiction.edit_approval==1"
                         @click.stop="approver_del(index)"></i>
                    </div>
                    <div class="name"
                         v-if="item.type==3"
                         :title="item.info[0].name">{{item.info[0].name}}</div>
                    <div class="name"
                         v-else
                         :title="item.title">{{item.title}}</div>

                    <div class="other"
                         v-if="item.type!=3||item.info.length>1">{{item.approver_type==1?'（会签）':'（或签）'}}</div>
                  </div>
                  ...
                  <div class="box"
                    ...
                    <div class="ellipsis"
                         ...
                         @click="add_approval(1,index+1)">. . .</div>

                    <div class="add-user insert"
                         v-show="index+1!==approved.length"
                         @click="add_approval(1,index+1)">+</div>
                  </div>
              ...
              <div class="person-list">
                <div class="person-item"
                  ...
                  <div class="msg">
                    <div class="user-img">
                      <div class="head-img">
                        <img :src="item.avatar||require('../../../../assets/adddraft/default-head-img.png')"
                             alt />
                      </div>

                      <i class="x hand"
                         v-if="process.process_jurisdiction&&process.process_jurisdiction.edit_cclist==1"
                         @click="CCitemClose(index)"></i>
                    </div>
    ...
    <div class="line red-template">
      ...
      <div class="s-content">
        <div class="s-c-title">
          <div class="flex-align-center">
            当前套红模板：
            <span class="that-process"
                  v-if="Getredtemplat.name">{{Getredtemplat.name}}</span>

            <span class="set-collection hand"
                  @click="set_red_template"
                  :class="{'collection':Getredtemplat.is_default==1}"
                  v-show="Getredtemplat.name">
              <i class="collection-no"></i>
              <span v-if="Getredtemplat.is_default==0">设为我的常用模板</span>
              <span v-else>设为我的常用模板</span>
            </span>
          </div>

          <span class="upload-template"
                @click="set_new_process(2)">
            <span>上传套红模板</span>
          </span>
        </div>

        <div class="s-c-content">
          <a-radio-group v-model="Getredtemplat.id">
            <div class="want">
              <div class="want-title">套红模板：</div>
              <div class="want-content">
                <div class="ofen-use-template"
                     v-if="all_data.redtemplete_list&&all_data.redtemplete_list.length>0">
                  <div class="template-item"
                       v-for="(item,index) in all_data.redtemplete_list"
                       :key="index">
                    <a-radio :title="item.name"
                             :value="item.id"
                             :style="Getredtemplat.id==item.id?'color:#0082ef;':''"
                             @click="red_template_change(item)">{{item.name}}</a-radio>
                    <div class="template-img">
                      <div class="mask">
                        <div class="look"
                             @click="look_red_template(item)"></div>
                      </div>
                      <img src="../../../../assets/adddraft/red-tepment.png"
                           alt />
                    </div>
                  </div>
                  <div class="more hand"
                       v-if="all_data.redtemplete_list.length > 20"
                       @click="more_show_template=!more_show_template">
                    <span>更多套红模板</span>
                    <i class="sanjiao"
                       :class="more_show_template?'shang':'xia'"></i>
                  </div>
                </div>
                <div class="more-template"
                     v-show="more_show_template && all_data.redtemplete_list.length > 20">
                  <a-input-search placeholder="搜索模板名称"
                                  style="width: 279px"
    ...
    <!-- 审批人弹窗 -->
    <a-modal class="transfer-modal appvisi-modal"
             title="选择审批人"
             :visible="appvisi"
      ...
      <div class="list-box-app">
        <div class="list-box-top">
          <a-radio-group v-model="appvisiradiovalue"
                         @change="appvisiradioonChange">
            <a-radio class="list-box-top-radio"
                     :value="1">上级（自动设置通讯录中的上级领导为抄送人）</a-radio>
            <a-radio class="list-box-top-radio"
                     :value="2">标签</a-radio>
            <a-radio class="list-box-top-radio"
                     :value="3">单个成员</a-radio>
            <a-radio class="list-box-top-radio"
                     :value="4">申请人</a-radio>
            <a-radio class="list-box-top-radio"
                     :value="5">部门</a-radio>
        ...
        <div class="list-box-bottom">
          <div v-show="appvisiradiovalue==1">
            ...
            <a-select size="default"
                      ...
                      style="width: 200px"
                      @change="handleChange">
              <a-select-option v-for="i in selectO"
                               :key="i.value">{{ i.title}}</a-select-option>
            </a-select>
            <div class="list-box-bottom-title">同级有多个上级时</div>
            <a-radio-group v-model="appvisiradioone.appvisiradiovalue">
              <a-radio class="list-box-top-radio"
                       :value="2">或签（一名上级同意即可）</a-radio>
              <a-radio class="list-box-top-radio"
                       :value="1">会签（须所有上级同意）</a-radio>
            </a-radio-group>
          </div>
          <div v-show="appvisiradiovalue==2">
            ...
            <Framelist :show_data_flag="appvisi"
    ...
    <a-modal title="保存草稿"
      ...
      <p>即将跳转到新建{{jumpPopup.centenText}}页面，是否需要保存本页面内容为草稿！！！</p>
    </a-modal>
    <!-- 抄送人弹窗 -->
    ...
    <!-- 套红预览 -->
    <a-modal class="transfer-modal red-model"
             title="套红模板预览"
      ...
      <div class="view_template">
        <span class="content"
              v-html="view_template.content"></span>
      </div>
    </a-modal>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      red_template: {
        ...
        red_template_model: null, //绑定的套红模板id
      },
  ...
  created() {
    if (
      this.all_data.technological_process_id ||
      this.all_data.technological_process_id == 0 ||
      this.get_process.liucheng_id
    ) {
      if (this.all_data) {
        this.get_approval_before_process()
      }
    }

    //向vuex提交数据
    this.$bus.$off('updata_approvals')
    this.$bus.$on('updata_approvals', this.updata_approval_vuex)

    //当部门改变时，重新获取审批人
    this.$bus.$off('update_approval_data')
    this.$bus.$on('update_approval_data', () => {
      this.get_approval_before_process_change()
    })
  ...
  methods: {
    ...
    onChangeId(e) {
      this.all_data.redtemplete_list.forEach((item) => {
    ...
    async get_approval_before_process() {
      ...
      if (
        ...
        approval_id = this.all_data.technological_process_id
      } else {
      ...
      let { data } = await approval_before_process({
        approval_id,
        dep_id: this.all_data.dept?.[0]?.id || '',
      })
      if (data.data) {
        data.data.forEach((el) => {
          //唯一的标识
          el.del_id = this.del_prefix[el.type] + el.id
          if (el.info && el.info.length > 0) {
            el.info.forEach((el2) => {
              el2.id = el2.userid
              el2.flag = 3
            })
        ...
        this.$set(this.process, 'that_process', data.explain)
        this.$set(this.process.that_process, 'line', data.collect)
        this.$set(
      ...
      // 给footer组件传流程数据
      this.$bus.$emit('updata_footer_process', data)
    },

    //获取流程信息（当流程id改变的额时候）
    async get_approval_before_process_change() {
      let { data } = await approval_before_process({
        approval_id: this.get_process.liucheng_id,
      ...
      if (data.approval_copy && data.approval_copy.length) {
        data.approval_copy.forEach((el) => {
          el.users = [
            {
              flag: 3,
      ...
      //叫footerhandle组件更新即将流向
      this.$bus.$emit('updata_next_process', data)
      //叫footer组件更新流程
      this.$bus.$emit('updata_footer_process', data)
      this.approved = JSON.parse(JSON.stringify(data.data || ''))
      this.CCroved = JSON.parse(JSON.stringify(data.approval_copy || ''))
    ...
    async get_approval_my_template(limit = 3) {
      ...
      if (data.data) {
        data.data.forEach((el) => {
          if (el.line === 1) {
            this.process.ofen_use.push(el)
          }
        })
        let has_id = []

        this.process.ofen_use.forEach((el, index) => {
          if (has_id.includes(el.id)) {
            this.process.ofen_use.splice(index, 1)
          } else {
            has_id.push(el.id)
          }
        })
    ...
    // 设置显示的当前流程
    process_change(item) {
      this.process.that_process = JSON.parse(JSON.stringify(item))
    },
    //流程改变
    liucheng_change() {
      this.get_approval_before_process_change()
      this.updata_approval_vuex()
    },
    //设置常用流程、反向
    async process_collecting() {
      let collect = null

      switch (parseInt(this.process.that_process.line)) {
        case 0:
          collect = 1
          break
        case 1:
      ...
      this.process.that_process = data
    },
    ...
    async set_red_template() {
      ...
      switch (this.Getredtemplat.is_default) {
        case 0:
          collect = 1
          break
        case 1:
      ...
      let { data, code } = await red_set_commonly({
        id: this.Getredtemplat.id,
        status: collect,
        templete_id: parseInt(
          this.$route.query.id || this.all_data.templete_id
        ),
        type: this.$route.query.type,
      })
      data.forEach((el) => {
        if (el.id == this.Getredtemplat.id) {
    ...
    upload_red_template() {
      let path = null
      if ([1, 2, 3].includes(parseInt(this.$route.query.type))) {
        path = parseInt(this.$route.query.type)
        switch (path) {
          case 1:
            path = '/postcrimsonset/crimsonadd?type=1'
            break
          case 2:
            path = '/getcrimsonset/getcrimsonadd?type=1'
            break
          case 3:
            path = '/signoffcrimsonset/crimsonadd?type=1'
            break
        }
      } else if (parseInt(this.$route.query.type) == 6) {
    ...
    //提交数据到vuex
    updata_approval_vuex() {
      this.$store.commit('SET_APPROVAL_approval_where', this.approved)
      this.$store.commit('SET_APPROVAL_approval_copy', this.CCroved)
    },
    //流程搜索
    ...
    apphandleOk() {
      ...
      this.$bus.$emit('updata_footer_process', this.approved, 1)

      this.$bus.$emit('updata_next_process', this.approved)
    ...
    CChandleOk() {
      for (let vel of this.CCpersonradio.tagsrange.visual) {
        // 前端type  1组织架构(部门)，2组织架构(人)，3发文收文单位，4标签，5角色，6最近选择
    ...
    // 接收点击编辑之后传来的参数
    GetMaindata() {
      this.$bus.$off('postflowpathsetSetdata')
    ...
    // 根据type判断要跳转的目的地
    async JumpFunction() {
      let path = parseInt(this.$route.query.type)
      switch (path) {
        case 1:
