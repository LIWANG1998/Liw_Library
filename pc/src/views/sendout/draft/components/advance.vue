<template>
  <div class="advance">
    <div class="advance-topbtnbox flex-col">
      ...
      <a-button v-if="powers('post_print')" @click="batchPrinting(1)">批量打印</a-button>
    </div>
      ...
      >
        ...
        <span slot="operation" slot-scope="item,record">
          <div class="flex-col operation">
            <!-- <div class="hand mr10" style="color:#0082ef;" @click="editclick(record,1)">编辑</div> -->
            <div
              class="hand"
              v-if="powers('post_delete')"
              style="color:#F56C6C;"
              @click="editclick(record,2)"
            >删除</div>
          </div>
        </span>
      </a-table>
...
import { document_information } from "@/api/draft";
...
export default {
  ...
  mounted() {
    this.height = Calculatedaltitude(40, 48, 60, 85);
    // tabs 改变就关闭高级搜索
    setTimeout(() => {
      this.$bus.$on("postBoxList", () => {
  ...
  methods: {
    ...
    onClicks(record, index) {
      return {
        on: {
          click: () => {
            let go_path = "";
            go_path = `home/incoming?id=${record.id}&file_type=1`;
            this.$router.push({
              path: go_path
            });
          }
    ...
    // 新建按钮
    transfer() {
      this.$router.push("draft/quickPost");
    },
    // 关闭模态框
    delNewDoucument() {
    ...
    getlibraryList() {
      ...
      libraryList({
        type: 1, // 1发文，2收文，3签报
        status: 6, // 1待审 2驳回 3完成 4发布 5草稿 6预发文 10全部
        ...
        advanced: {
          copy_users: obj.one["发文单位"]?.intext, // 发文单位
