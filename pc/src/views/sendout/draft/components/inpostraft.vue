<template>
  <div class="inpostraft">
    ...
    <div class="flex-col bigbox" v-if="searchdata.length">
      <div class="inpostraft-inbox" v-for="item in searchdata" :key="item.id">
    ...
    <div class="flex-col bigbox" v-if="!searchdata.length && data.length">
      <div class="inpostraft-inbox" v-for="item in data" :key="item.id">
        <p class="inpostraft-inbox-title" :title="item.temp_name">
          {{ item.temp_name }}
        </p>
        <div class="inpostraft-inbox-title-box">
    ...
    <div class="flex-col bigbox" v-if="!searchdata.length">
      <div class="inpostraft-inbox" v-for="item in common" :key="item.id">
...
export default {
  ...
  methods: {
    ...
    // 获取公文模板
    getselectlist() {
      GetselectList({ kwd: this.form.name, type: 1, page: 1 }).then((res) => {
