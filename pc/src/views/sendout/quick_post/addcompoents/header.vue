  ...
  data() {
    return {
      ...
      tags: [
        ...
        { tab: false, bm: true, company: true, hl: true }, //（发文收文单位(单位)）
  ...
  methods: {
    ...
    handleOk_ren() {
      ...
      if (this.that_e_name == "main_delivery") {
        this.select_data.reviewer.length > 0
          ...
          if (el.receiver) {
            ...
            if (this.select_data.reviewer) {
              el.receiver.forEach((item) => {
                ...
                this.all_data.reviewer.push(el.title + "-" + item.name);
              });
              this.all_data.reviewer = Array.from(
                new Set(this.all_data.reviewer)
              );
    ...
    set_vuex_data() {
      for (let key in this.select_data) {
        ...
        if (key == "main_delivery") {
          ...
          console.log(this.$store.state.quick_draft.header.main_delivery);
        } else {
          this.$store.commit(mothods_name, {
    ...
    main_delivery_deselect(val) {
      this.select_data.main_delivery.forEach((el, inx) => {
        if (el.name == val) {
          ...
          let receiver_v = this.select_data.main_delivery[inx].id;
          ...
          //   if (el2.id == receiver_v) {
          //     this.select_data.reviewer.splice(inx2, 1);
          //     this.all_data.reviewer.splice(inx2, 1);
          //   }
          // });
          for (
            let index = this.select_data.reviewer.length - 1;
            index >= 0;
            index--
          ) {
            console.log(this.select_data.reviewer);
            if (this.select_data.reviewer[index].id == receiver_v) {
              this.all_data.reviewer.forEach((ite, dex) => {
                if (ite === this.select_data.reviewer[index].name) {
                  this.all_data.reviewer.splice(dex, 1);
                }
              });
              this.select_data.reviewer.splice(index, 1);
            }
          }
