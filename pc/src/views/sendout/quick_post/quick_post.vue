...
export default {
  ...
  created() {
    ...
    //意外退出保存草稿
    this.$bus.$on("SettingNotice", this.preservations);
  },
  mounted() {
    window.scrollTo(0, 0);
  },
    ...
    has_head_data() {
      ...
      document_arr.forEach((el) => {
        if (el.value) {
          if (el.value.select) {
            if (
              el.value.select.length > 0 ||
              el.value.text.length > 0 ||
              el.value.inter.length > 0
            ) {
              judge.push({
                flag: true,
                c_name: el.c_name,
  ...
  methods: {
    ...
    back() {
      ...
      this.$confirm({
        ...
        onOk() {
          //叫发文保存草稿
          _that.go_document_add(8, 1, "NoJump");
          _that.$router.go(-1);
        },
        onCancel() {
          _that.$router.go(-1);
