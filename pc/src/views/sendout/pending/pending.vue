<template>
  <div class="pending">
    ...
    <Maincontent>
      <a-tabs default-active-key="1" @change="onCallback">
        <a-tab-pane key="1">
          <span slot="tab"> <SvgIcon item="myApproval" />待我审的 </span>
        </a-tab-pane>
        <a-tab-pane key="2">
          <span slot="tab"> <SvgIcon item="ApprovalOthers" />待别人审的 </span>
        </a-tab-pane>
      </a-tabs>
      <div class="head">
        <div class="left">
        ...
        <div class="right">
          <a-button class="btn" @click="onJump" type="primary">新建</a-button>
          <a-icon type="more" />
          <a-button class="btn" @click="addDocumentExcelout" :disabled="comIds"
            >导出</a-button
          >
          <a-button
            v-if="powers('post_print')"
            class="btn"
            @click="batchPrinting(1)"
            >批量打印</a-button
          >
        </div>
      </div>
        ...
        >
          ...
          >
            <span slot="process_urgent_level" slot-scope="item, record">
              <span id="degree" :class="record.ClassType">{{
                record.process_urgent_level
              }}</span>
            </span>
            <span slot="status" slot-scope="item, record">
              <span
                id="status"
                :style="
                  [1, 2].includes(record.r_status)
                    ? 'color:#0082EF;'
                    : 'color:rgb(245, 172, 64);'
                "
              >
...
export default {
  ...
  data() {
    return {
      ...
      SearchTriangle: true,
      // 模板弹窗数据
      TemplatePopup: { visibles: false, type: 1, title: "发文模板", type2: 1 },
      table_data: {
        order_field: null,
        sort: null,
      },
  ...
  mounted() {
    this.height = Calculatedaltitude(60, 36, 100, 61);
  },
  methods: {
    // tabs切换事件
    onCallback(id) {
    ...
    addlist() {
      ...
      document_wait_approval_list({
        ...
        advanced: {
          ...
          copy_users: obj.one["发文单位"]?.intext, // 发文、签报单位
