<template>
  <div class="acceptadd">
    ...
    <Maincontent>
      <div class="flex-col acceptadd-box">
        ...
        <template v-for="(item, index) in view_data">
          <div class="form-inline" :key="index">
            ...
            >{{item.value|urgen_level(datas.degree)}}</div>
            <div
              v-else-if="item.e_name=='urgent_level'"
              class="form-inline-input heights"
            >{{item.value|Degree(datas.urgen_level)}}</div>
      ...
      <div class="flex-col acceptadd-box">
        ...
        <div class="zhengwen">
          ...
          <div class="zhang-list">
            <img
              class="zhang-item"
              :style="'z-index:'+item.zindex+'px;top:'+item.position.top+'px;left:'+item.position.left+'px;width:'+item.position.width+'px'"
      ...
      <div class="flex-col acceptadd-box">
        ...
        <div class="twoform addInpostrafts-enclosure">
          <div class="form-inline-input addInpostrafts-enclosure-textbox">
            ...
            >
              <div class="left">
                <div :class="get_file_img(item.path.split('.')[item.path.split('.').length - 1])"></div>
                <div class="msg">
                  <span class="addInpostrafts-enclosure-textbox-inbox-tltle-text">{{item.old_name}}</span>
                  <span
        ...
        'jpg',
        'bmp',
      ].includes(item.path.split('.')[item.path.split('.').length - 1])"
      ...
      <a-modal class="acceptGoBack" v-model="goBack.visible" title="退回理由" @ok="onHandleOk">
        <a-textarea
    ...
    <a-affix :offset-bottom="15">
      <div
        ...
        v-if="buttons.zhuanshouwen||buttons.tuihui"
      >
        <div class="flex-col-bd"></div>
        <a-button @click="transfer" class="bcbtn" v-if="buttons.zhuanshouwen">转收文</a-button>
        <a-button @click="onGiveBack" class="bcbtn" v-if="buttons.tuihui">退回</a-button>
      </div>
    </a-affix>
    <NewDocument :TemplatePopup="TemplatePopup" @delNewDoucument="delNewDoucument" />
    <a-modal
...
export default {
  ...
  methods: {
    ...
    getGiveBack() {
      addgiveBack({
        id: this.TemplatePopup.distribute_id,
