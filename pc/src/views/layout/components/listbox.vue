    ...
    >
      <template v-for="itme in data">
        ...
        <template v-else v-for="info in itme.childrens">
          <a-menu-item
            v-if="info.code == 'system_role_left' && powers[8] != ''"
            :key="info.code + 1"
          >
          ...
          <a-menu-item :key="info.path">
            <div
              class="listbox-name-dian"
              v-if="remindOnOff && info.path == '/accept'"
            ></div>
            <router-link :to="info.path">{{ info.name }}</router-link>
          </a-menu-item>
          <a-menu-item
            v-if="info.code == 'system_role_left' && powers[6] != ''"
            :key="info.code + 2"
          >
...
export default {
  ...
  methods: {
    ...
    working(power, item) {
      ...
      if (code != "yes") {
        // 互联企业判断权限还要判断是否是wx3
        if (code == "system_inter_left") {
          yseno = power.includes(code) && this.userinfo.wx3 == 1;
        } else {
