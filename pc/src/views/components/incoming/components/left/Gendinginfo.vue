...
export default {
  ...
  data() {
    return {
      ...
      tags: {
        ...
        2: { tab: false, bm: true, company: true, hl: true }, //（发文收文单位(单位)）
  ...
  methods: {
    ...
    show_organizate(val, item) {
      ...
      item.e_name == "receiving_unit" || item.e_name == "main_delivery"
        ? (this.tag.hl = true)
        : "";
    ...
    //组织架构确认
    handleOk_ren() {
      console.log(this.visiblerange.visual, this.that_e_name);
      //格式变化
      //主送单位
      //设置当在弹窗内删除数据后，在外面也要删除
      this.before_input_arr = this.data_screen(this.that_e_name);
      this.select_data[this.that_e_name] =
        typeof this.select_data[this.that_e_name] == "object" &&
        this.select_data[this.that_e_name].length > 0
          ? [...this.select_data[this.that_e_name]]
          : [];
      this.all_data[this.that_e_name] = this.all_data[this.that_e_name] =
        typeof this.all_data[this.that_e_name] == "object" &&
        this.all_data[this.that_e_name].length > 0
          ? [...this.all_data[this.that_e_name]]
          : [];
      if (this.that_e_name == "main_delivery") {
        // ---------------------这个地方有问题--------------------
        let revi =
          this.that_e_name == "main_delivery" ? "reviewer" : "receiving_user";
        this.select_data[revi] =
          typeof this.select_data[revi] == "object" &&
          this.select_data[revi].length > 0
            ? [...this.select_data[revi]]
            : [];
        //设置接收人
        this.visiblerange.visual.forEach((el) => {
          if (el.receiver_name) {
            if (
              this.select_data[revi] &&
              [1, 2].includes(parseInt(this.$route.query.file_type)) &&
              el.receiver_name
            ) {
              this.select_data[revi].push({
                id: el.value,
                name: el.receiver_name,
                wid: "dw" + el.value,
              });
              this.all_data[revi].push(el.receiver_name);
              this.all_data[revi] = Array.from(new Set(this.all_data[revi]));
            }
          }
          // ---------------------------
          if (el.receiver) {
            if (this.select_data[revi]) {
              el.receiver.forEach((item) => {
                this.select_data[revi].push({
                  id: el.value,
                  name: el.title + "-" + item.name,
                  wid: "hl" + el.value,
                });
                this.all_data[revi].push(el.title + "-" + item.name);
              });
              this.all_data[revi] = Array.from(new Set(this.all_data[revi]));
            }
          }
        ...
        this.select_data.receiving_user =
          typeof this.select_data.receiving_user == "object" &&
          this.select_data.receiving_user.length > 0
            ? [...this.select_data.receiving_user]
            : [];
        //设置接收人
        this.visiblerange.visual.forEach((el) => {
          ...
          if (el.receiver) {
            if (
              ...
              el.receiver.forEach((item) => {
                ...
                this.all_data.receiving_user.push(el.title + "-" + item.name);
              });
              this.all_data.receiving_user = Array.from(
                new Set(this.all_data.receiving_user)
              );
    ...
    set_vuex_data() {
      for (let key in this.select_data) {
        ...
        // ---------------------z-------------------
        if (key == "main_delivery" || key == "receiving_unit") {
          let select = [];
          let inter = [];
    ...
    head_data_deselect(e_name, option) {
      if (e_name == "main_delivery" || e_name == "receiving_unit") {
        this.main_delivery_deselect(option, e_name);
      } else {
    ...
    //主送单位删除
    main_delivery_deselect(val, nem) {
      let _this = this;
      let revi = nem == "main_delivery" ? "reviewer" : "receiving_user";
      this.select_data[nem].forEach((el, inx) => {
        if (el.name == val) {
          let receiver_name = _this.select_data[nem][inx].receiver_name;
          let receiver_v = _this.select_data[nem][inx].id;
          _this.select_data[nem].splice(inx, 1);
          _this.all_data[nem].splice(inx, 1);
          for (
            let index = _this.select_data[revi].length - 1;
            index >= 0;
            index--
          ) {
            console.log(_this.select_data[revi]);
            if (_this.select_data[revi][index].id == receiver_v) {
              _this.all_data[revi].forEach((ite, dex) => {
                if (ite === _this.select_data[revi][index].name) {
                  _this.all_data[revi].splice(dex, 1);
                }
              });
              _this.select_data[revi].splice(index, 1);
            }
          }
        }
      });
    ...
    get_data() {
      if (this.leftdata_fields) {
        ...
        for (let key in this.leftdata_fields) {
          ...
          if (
            this.leftdata_fields[key]?.value?.inter &&
            this.leftdata_fields[key].value.inter.length > 0
          ) {
            this.select_data[this.leftdata_fields[key].e_name] = JSON.parse(
              JSON.stringify(this.leftdata_fields[key].value.inter)
            );

            this.leftdata_fields[key].value.inter.forEach((el) => {
              all_value.push(el.name);
            });
          }
