<template>
  <div class="summary">
    <ul class="sec-tab">
      <li :class="{ active: summary_tab === 0 }" @click="summary_tab = 0">
        审批意见
      </li>
      <li :class="{ active: summary_tab === 1 }" @click="summary_tab = 1">
        传阅意见
        <span v-if="crculation.remark">({{ crculation.remark.length }})</span>
      </li>
    </ul>
    <ul class="sec-content">
      <li class="approval" v-show="summary_tab === 0">
        <div class="r-content">
          <div
            class="opinion-item"
          ...
          >
            ...
            <div class="date mb flex-col-db">
              <div class="flex-align-center">
                <span class="jiedian" v-if="index <= 0">{{ item.name }}</span>
                <span class="submitter" v-if="index > 0" :title="item.name">{{
                  item.name
                }}</span>
                <span v-if="item.status == 2" class="approver_type">{{
            ...
            <div class="peoples" :style="item.is_show ? 'height:0px;' : ''">
              <template v-for="(item2, index2) in item.userinfo">
                <div class="people" :key="index2">
                  ...
                  <div class="enclosure" v-if="filelength(item2)">
                    <span style="color: #909399">
                      <SvgIcon
                        :item="'enclosure'"
                        style="color: #c0c4cc; margin-right: 5px"
                      />附件
                    </span>
                    <div>
                      <a
                        class="download"
                        :href="`${HttpUrl}/enclosure_download?user_id=${item2.userid}&document_id=${$route.query.id}&type=4&token=${tokens}`"
                        download
                      >
                ...
                <template v-for="(item3, index3) in item2.transfer">
                  <div class="people" :key="'transfer' + index3">
                    <!-- <div class="date mb flex-db">
                  <span>{{item2.create_time}}</span>
                </div> -->
                    <div class="user-msg mb flex-col">
                      <SvgIcon
                        item="returns"
                        style="color: #839ccf; padding-right: 10px"
                      />
                      <div class="head-img">
                        <img :src="item3.avatar" alt />
                      </div>
                      <div class="name">
                        {{ item3.name }} {{ item3.position }}
                      </div>
                      <div class="flex-col-bd"></div>
                      <span
                        class="approval-top-dian"
                        :style="`color:${objstatus[item3.rstatus].color}`"
                        >·</span
                      >
                      <span
                        class="approval-top-stuts"
                        :style="`color:${objstatus[item3.rstatus].color}`"
                        >{{ objstatus[item3.rstatus].text }}</span
                      >
                    </div>
                    <div
                      class="opinion mb"
                    ...
                    >
                      <!-- <span class="opinion-agree">{{item2.c_status}}</span> -->
                      <span class="flex-align-center">{{ item3.explain }}</span>
                    </div>

                    <div class="enclosure" v-if="filelength(item3)">
                      <span style="color: #909399">
                        <SvgIcon
                          :item="'enclosure'"
                          style="color: #c0c4cc; margin-right: 5px"
                      ...
                      <div>
                        <a
                          class="download"
                          :href="`${HttpUrl}/enclosure_download?user_id=${item3.userid}&document_id=${$route.query.id}&type=4&token=${tokens}`"
                          download
                        >
                          <SvgIcon :item="'dowannex'" />
                        </a>
      ...
      <li class="crculation" v-show="summary_tab === 1">
        <div class="title" v-if="crculationremark(crculation)">传阅意见</div>
        <div class="r-content">
          <div class="opinion-list">
            ...
            >
              ...
              <div class="opinion mb" v-if="item.remark">
                <span class="opinion-agree">{{ item.remark }}</span>
              </div>
            </div>
          </div>
        </div>
        <div class="title record" v-if="get_leftdata.button.yijian">
          我的意见
        </div>
        <div v-if="get_leftdata.button.yijian" class="content">
          ...
          <div class="content-btn">
            <a-button type="primary" @click="clickprimary">确定</a-button>
          </div>
        </div>

        <NotFound
          v-show="crculation.remark.length <= 0 && !get_leftdata.button.yijian"
        />
      </li>
    </ul>
  </div>
        ...
        data.data.forEach((el) => {
          if (el.userinfo.length > 0) {
            el.userinfo.forEach((el2) => {
              ...
              if (el2.transfer && el2.transfer.length > 0) {
                el2.transfer.forEach((el3) => {
                  switch (el3.rstatus) {
                    case 3:
                      el2.c_status = "同意";
                      break;
