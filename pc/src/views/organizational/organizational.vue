  ...
  <div>
    ...
    <Maincontent>
      ...
      <div class="maincontent-box maincontent-right">
        <Header :title="HeaderTitle">
          ...
          <a-button @click="onNewlyAdded" class="maincontent-newly-added"
            >角色管理</a-button
          >
          <a-button @click="onSynchro" v-if="powers('organization_synchronous')"
            >同步组织架构</a-button
          >
        </Header>
        <!-- 新加的已选用户和角色选择，关键词的搜索 -->
        <div class="flex-col">
          <div class="organizational-box">
            已选
            <span class="organizational-box-num">{{ Keys.length }}</span
            >位用户
          </div>
          <a-button
            :disabled="disabled"
    ...
    <a-modal title="同步组织架构" v-model="modal.visible">
      ...
      <p class="upload-title flex-col">
        ...
        <span class="upload-title-right hand" v-if="is_wx3" @click="onSecret"
          >如何获取通讯录secret?</span
        >
      </p>
      <a-input
    ...
    >
      <template slot="footer">
        <a-button key="submit" type="primary" @click="onSecretgb"
          >关闭</a-button
        >
      </template>
      <a-carousel arrows>
    ...
    uploadOk() {
      ...
      if (_this.onOff) {
        ...
        OrganiZational({ link_secret: _this.modal.kwd }).then((res) => {
          ...
          this.addListnum = 0;
          this.treeClickid = "";
          this.$refs.treeTemplate.onReset();
          // 获取部门和人员列表
          this.addList();
          // 获取角色信息列表
          this.roleList();
