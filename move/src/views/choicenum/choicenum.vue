...
export default {
  ...
  computed: {
    ...
    item() {
      return this.$store.state.edittext.document_arrs[this.$route.query.key];
    },
  },
  methods: {
    // 搜索
    ...
    determine() {
      if (JSON.stringify(this.choiserule) == "{}") {
        ...
        this.iss.value = [];
        if (this.radio != 0) {
          this.iss.value.push(JSON.parse(JSON.stringify(this.choiserule)));
        }
      }
      this.statusclick();
      this.$router.go(-1);
    ...
    statusclick(t) {
      ...
      this.isss.id = this.iss.value[0]?.id ? this.iss.value[0]?.id : "";
      this.isss.name = this.iss.value[0]?.name ? this.iss.value[0]?.name : "";
      this.isss.word_no = this.iss.value[0]?.word_no
          ? this.iss.value[0]?.word_no
          : "";
    },
    //选择的值
    changeRule(item) {
  ...
  activated() {
    ...
    this.radio = this.iss.value.length > 0 ? this.iss.value[0].id : 0;
