<template>
  <div class="check-elesign">
    <div class="check-elesign-top flex-ac">
      <van-search v-model="data.name" placeholder="请输入关键词" input-align="center" />
    </div>
    <div class="check-elesign-top-tab">
      <van-tabs v-model="data.status">
      ...
      <van-list v-model="loading" :finished="finished" @load="onLoad">
        <template v-for="(item, index) in list">
          <div class="ele-box" :key="index" @click="ckeckeleFun(item)">
            ...
            <div class="pass_cell_show" v-if="eleItem && item.id==eleItem.id">
    ...
    <div class="check-elesign-bottom flex-ac-jsb">
      <van-button type="default" class="check-elesign-bottom-btn" @click="cencel">取消</van-button>
      <van-button type="info" class="check-elesign-bottom-btn" @click="istOK">确认</van-button>
    </div>
    <!-- 选择电子 使用密码签章 -->
    <van-popup v-model="ispass" position="bottom" class="pwsbox">
      ...
      <div class="bottom-btn">
        <van-button type="default" class="btn" @click="cencelpass">取消</van-button>
        <van-button type="info" class="btn" style="margin-left:12px" @click="isOKpass">确认</van-button>
      </div>
    </van-popup>
  </div>
</template>
